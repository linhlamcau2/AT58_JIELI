...
static void rd_K9B_flash_delete(uint32_t Mac_K9B_save)
{
	...
	else
	{
		...
		if (array_of_MAC_delete < (MAX_NUM_K9ONOFF - 1))
		{ // not end array
...
uint8_t RD_K9B_check_saveAndDelete(uint32_t macDevice, uint32_t counter, uint8_t type_k9b, uint8_t button_id)
{
	...
	if ((button_id != 0b110) && (button_id != 0b011))
	{ // press button other than 1 and 3 will resset count clear and count pair
		count_clear = 0;
		count_pair = 0;
	}
	macDevice_last = macDevice;
